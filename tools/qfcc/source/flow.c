...
flowvar_add_amb_use (flowvar_t *var, statement_t *st)
{
	...
	if (var->op->op_type == op_temp) {
		...
		if (def) {
			def_visit_all (def, dol_sub, flowvar_def_add_amb_use, st);
		}
	}
}
...
flowvar_add_amb_def (flowvar_t *var, statement_t *st)
{
	...
	if (var->op->op_type == op_temp) {
		...
		def_t      *def = var->op->def;
		if (def) {
			def_visit_all (def, dol_sub, flowvar_def_add_amb_def, st);
		}
	}
}
...
static void
flow_count_record (statement_t *st, int var, reaching_t *r)
{
	r->num_ud_chains += set_count (r->tmp);
}

static void
flow_record_start (statement_t *st, reaching_t *r)
{
	r->first_use[st->number] = r->num_ud_chains;
	r->num_use[st->number] = 0;
	set_add (r->st_update, st->number);
}

static void
flow_record_record (statement_t *st, int var, reaching_t *r)
{
	for (set_iter_t *ud = set_first (r->tmp); ud;
		 ud = set_next (ud)) {
		set_add (r->udchains[var], r->num_ud_chains);
		udchain_t  *udc = &r->ud_chains[r->num_ud_chains++];
		udc->var = var;
		udc->usest = st->number;
...
static void
flow_record_end (statement_t *st, reaching_t *r)
{
...
flow_chain_core (flowgraph_t *graph, reaching_t *reach,
				 ...
				 void (*record) (statement_t *st, int var, reaching_t *r),
				 void (*end) (statement_t *st, reaching_t *r))
{
...
flow_build_chains (flowgraph_t *graph)
{
	...
	while (1) {
		...
		graph->func->ud_chains = reach.ud_chains;
		graph->func->num_ud_chains = reach.num_ud_chains;
	}
	...
	if (reach.num_ud_chains) {
		memcpy (graph->func->du_chains, graph->func->ud_chains,
				sizeof (udchain_t[reach.num_ud_chains]));
...
flow_analyze_pointer_operand (operand_t *ptrop, set_t *def)
{
	...
	if (ptrop->op_type == op_value && ptrop->value->lltype == ev_ptr) {
		...
		if (op) {
			flow_add_op_var (def, op, dol_sub);
		}
	}
	return op;
}
						...
						operand_t *operands[FLOW_OPERANDS])
{
	...
	if (kill) {
		...
		for (operand_t *op = s->kill; op; op = op->next) {
			flow_add_op_var (kill, op, dol_sub);
		}
	}
	if (operands) {
	...
	switch (s->type) {
		...
		case st_address:
			...
			flow_add_op_var (def, s->opc, dol_sub);
			if (operands) {
				operands[0] = s->opc;
				operands[1] = s->opa;
				operands[2] = s->opb;
		...
		case st_expr:
			flow_add_op_var (def, s->opc, dol_sub);
			flow_add_op_var (use, s->opa, dol_sub);
			if (s->opb)
				flow_add_op_var (use, s->opb, dol_sub);
			if (operands) {
				operands[0] = s->opc;
				operands[1] = s->opa;
				operands[2] = s->opb;
			...
			break;
		case st_assign:
			flow_add_op_var (def, s->opa, dol_sub);
			flow_add_op_var (use, s->opc, dol_sub);
			if (operands) {
				operands[0] = s->opa;
				operands[1] = s->opc;
			}
		...
		case st_ptrmemset:
			...
			if (!strcmp (s->opcode, "move")
				...
				flow_add_op_var_size (def, s->opc, size, dol_sub);
				src_op = s->opa;
				res_op = s->opc;
			} else if (!strcmp (s->opcode, "movep")) {
				flow_add_op_var_size (use, s->opc, size, dol_sub);
