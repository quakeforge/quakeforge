...
designator_field (const designator_t *des, const type_t *type)
{
	...
	symtab_t   *symtab = type_symtab (type);
					 ...
					 const expr_t *eles, int base_offset)
{
	...
	if (is_algebra (type)) {
		...
		type = t;
			   ...
			   || (is_nonscalar (type) && type_symtab (type))) {
		state.field = type_symtab (type)->symbols;
				 ...
				 element_chain_t *element_chain)
{
	...
	for (element = element_chain->head; element; element = element->next) {
		...
		const expr_t *alias = _new_offset_alias_expr (type, init, offset);
