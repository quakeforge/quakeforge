...
typedef struct edge_extra_t
  {
    ...
    vertex_t  ref_faces_normal[MAX_REF_FACES]; /* normal of referenced faces   */
...
typedef struct options_t
  {
    ...
    int         z_direction;
    int         camera_axis; /* 1 - X, 2 - Y, 3 - Z, negatives will come from negative side of axis*/
...
void plotpoint(image_t *image, long xco, long yco, unsigned int color)
  {
    unsigned int      bigcol=0;

    if(xco < 0 || xco > image->width || yco < 0 || yco > image->height) return;

    bigcol=(unsigned int)image->image[yco * image->width + xco];

    bigcol=bigcol + color;

    if (bigcol < 0) bigcol=0;
    if (bigcol > 255) bigcol=255;
...
void bresline(image_t *image, long x1, long y1, long x2, long y2, unsigned int color)
  {
    long x=0, y=0;
        ...
        if(arg[0] == '-')
          {
            ...
            switch(arg[1])
              {
                case 's' : if(sscanf(&arg[2],"%ld",&lnum) == 1)
                             if (lnum > 0)
                               locopt.scaledown = (float)lnum;
                           break;

                case 'z' : if(sscanf(&arg[2],"%ld",&lnum) == 1)
                             if (lnum >= -1)
                               locopt.z_pad = (float)lnum;
                           break;

                case 'p' : if(sscanf(&arg[2],"%ld",&lnum) == 1)
                             if (lnum >= 0)
                               locopt.image_pad = (float)lnum;
                           break;

                case 'd' : if(sscanf(&arg[2],"%ld",&lnum) == 1)
                             if (lnum >= 0 && lnum <= 7)
                               locopt.z_direction = (int)lnum;
                           break;

                case 'c' : if(strlen(&arg[2]) == 2)
                             {
                               pm = arg[2];
                               axis = arg[3];
                               printf("-c%c%c\n",pm,axis);
                               switch(axis)
                                 {
                                   case 'x' :
                                   case 'X' : locopt.camera_axis=1;
                                              break;

                                   case 'y' :
                                   case 'Y' : locopt.camera_axis=2;
                                              break;

                                   case 'z' :
                                   case 'Z' : locopt.camera_axis=3;
                                              break;

                                   default  : printf("Must specify a valid axis.\n");
                                              show_help();
                                              exit(1);
                                              break;
                                 }
                               ...
                               switch(pm)
                                 {
                                   case '+' : break;
                                   case '-' : locopt.camera_axis=-locopt.camera_axis;
                                              break;
                                   default  : printf("Must specify +/-\n");
                                              show_help();
                                              exit(1);
                                              break;
                                 }
                           ...
                           break;
                case 't' : if(sscanf(&arg[2],"%f",&fnum) == 1)
                             if (fnum >= 0.0 && fnum <= 1.0)
                               locopt.flat_threshold = (float)fnum;
                           break;

                case 'e' : locopt.edgeremove = 0;
                           break;

                case 'n' : locopt.negative_image = 1;
                           break;

                case 'a' : if(sscanf(&arg[2],"%ld",&lnum) == 1)
                             if (lnum >= 0)
                               locopt.area_threshold = (int)lnum;
                           break;

                case 'l' : if(sscanf(&arg[2],"%ld",&lnum) == 1)
                             if (lnum >= 0)
                               locopt.linelen_threshold = (int)lnum;
                           break;

                case 'r' : locopt.write_raw = 1;
                           break;

                case 'u' : locopt.write_nocomp = 1;
                           break;
...
void show_options(struct options_t *opt)
  {
    ...
    switch (opt->z_direction)
      {
        case 0 : sprintf(dirstr,"up");
                 break;
        case 1 : sprintf(dirstr,"up & right");
                 break;
        case 2 : sprintf(dirstr,"right");
                 break;
        case 3 : sprintf(dirstr,"down & right");
                 break;
        case 4 : sprintf(dirstr,"down");
                 break;
        case 5 : sprintf(dirstr,"down & left");
                 break;
        case 6 : sprintf(dirstr,"left");
                 break;
        case 7 : sprintf(dirstr,"up & left");
                 break;
        default: sprintf(dirstr,"unknown!");
                 break;
      }
    ...
    switch (opt->camera_axis)
      {
        case 1  : sprintf(dirstr,"+X");
                  break;
        case -1 : sprintf(dirstr,"-X");
                  break;
        case 2  : sprintf(dirstr,"+Y");
                  break;
        case -2 : sprintf(dirstr,"-Y");
                  break;
        case 3  : sprintf(dirstr,"+Z");
                  break;
        case -3 : sprintf(dirstr,"-Z");
                  break;
        default : sprintf(dirstr,"unknown!");
...
int main(int argc, char *argv[])
{
  ...
  if (edgelist == NULL)
    {
      fprintf(stderr,"Error allocating %ld bytes for vertices.",sizeof(struct edge_t) * numedges);
      return 2;
    }
  ...
  */
      ...
      for (i=0; i<numfaces; i++)
        {
          ...
          while (vect.X == 0.0 && vect.Y == 0.0 && vect.Z == 0.0 && k < (facelist[i].ledge_num + j))
            {
              ...
              */
              ...
              if (ledges[j] > 0)
                {
                  v0.X = vertexlist[edgelist[abs((int)ledges[j])].vertex0].X -
                         vertexlist[edgelist[abs((int)ledges[j])].vertex1].X;
                  v0.Y = vertexlist[edgelist[abs((int)ledges[j])].vertex0].Y -
                         vertexlist[edgelist[abs((int)ledges[j])].vertex1].Y;
                  v0.Z = vertexlist[edgelist[abs((int)ledges[j])].vertex0].Z -
                         vertexlist[edgelist[abs((int)ledges[j])].vertex1].Z;

                  v1.X = vertexlist[edgelist[abs((int)ledges[k])].vertex0].X -
                         vertexlist[edgelist[abs((int)ledges[k])].vertex1].X;
                  v1.Y = vertexlist[edgelist[abs((int)ledges[k])].vertex0].Y -
                         vertexlist[edgelist[abs((int)ledges[k])].vertex1].Y;
                  v1.Z = vertexlist[edgelist[abs((int)ledges[k])].vertex0].Z -
                         vertexlist[edgelist[abs((int)ledges[k])].vertex1].Z;
                }
              else
                {
                  /* negative index, therefore walk in reverse order */
                  v0.X = vertexlist[edgelist[abs((int)ledges[j])].vertex1].X -
                         vertexlist[edgelist[abs((int)ledges[j])].vertex0].X;
                  v0.Y = vertexlist[edgelist[abs((int)ledges[j])].vertex1].Y -
                         vertexlist[edgelist[abs((int)ledges[j])].vertex0].Y;
                  v0.Z = vertexlist[edgelist[abs((int)ledges[j])].vertex1].Z -
                         vertexlist[edgelist[abs((int)ledges[j])].vertex0].Z;

                  v1.X = vertexlist[edgelist[abs((int)ledges[k])].vertex1].X -
                         vertexlist[edgelist[abs((int)ledges[k])].vertex0].X;
                  v1.Y = vertexlist[edgelist[abs((int)ledges[k])].vertex1].Y -
                         vertexlist[edgelist[abs((int)ledges[k])].vertex0].Y;
                  v1.Z = vertexlist[edgelist[abs((int)ledges[k])].vertex1].Z -
          ...
          //printf("%4ld - (%8.3f, %8.3f, %8.3f) X (%8.3f, %8.3f, %8.3f) = (%8.3f, %8.3f, %8.3f) -> ",i,v0.X, v0.Y, v0.Z, v1.X, v1.Y, v1.Z, vect.X, vect.Y, vect.Z);
          if (tempf > 0.0)
            {
              vect.X = vect.X / tempf;
              vect.Y = vect.Y / tempf;
          ...
          /* printf("<id=%ld|num=%ld>",facelist[i].ledge_id, facelist[i].ledge_num); */
  ...
  for (i=0;i<numvertices;i++)
    {
      ...
      switch(options.camera_axis)
        {
          case -1 : /* -X -- (-y <-->  +y, +x into screen, -x out of screen; -z down, +z up) */
          ...
          case -2 : /* -Y -- (+x <--> -x; -y out of screen, +z up) */
          ...
          case -3 : /* -Z -- negate X and Z (ie. 180 rotate along Y axis) */
                    vertexlist[i].X = -vertexlist[i].X;
      ...
      if (i == 0)
      ...
      else
        {
          ...
          if (vertexlist[i].Z < minZ) minZ=vertexlist[i].Z;
  ...
  image->width  = (long)((maxX - minX)/options.scaledown) + (options.image_pad*2) + (options.z_pad*2);
  image->height = (long)((maxY - minY)/options.scaledown) + (options.image_pad*2) + (options.z_pad*2);
  if(!(image->image=malloc(sizeof(eightbit) * image->width * image->height)))
    {
  ...
  else
    {
      ...
      memset(image->image,0,sizeof(eightbit) * image->width * image->height);
    }
  ...
  switch (options.z_direction)
    { case 0 : Z_Xdir=0;  /* up */
      ...
      case 1 : Z_Xdir=1;  /* up & right */
               Z_Ydir=-1;
               break;
      
      case 2 : Z_Xdir=1;  /* right */
               Z_Ydir=0;
               break;
      
      case 3 : Z_Xdir=1;  /* down & right */
               Z_Ydir=1;
               break;
      
      case 4 : Z_Xdir=0;  /* down */
               Z_Ydir=1;
               break;
      
      case 5 : Z_Xdir=-1; /* down & left */
               Z_Ydir=1;
               break;
      
      case 6 : Z_Xdir=-1; /* left */
               Z_Ydir=0;
               break;
      
      case 7 : Z_Xdir=-1; /* up & left */
               Z_Ydir=-1;
               break;
      
      default: Z_Xdir=1;  /* unknown - go with case 1 */
               Z_Ydir=-1;
               break;
    }
  ...
  for (i=0;i<=255;i++)
    {
      plotpoint(image,i,0,(255-i)); /* across from top left */
      plotpoint(image,0,i,(255-i)); /* down from top left */

      plotpoint(image,(image->width-i-1),0,(255-i)); /* back from top right */
      plotpoint(image,(image->width-1),i,(255-i)); /* down from top right */

      plotpoint(image,(image->width-i-1),(image->height-1),(255-i)); /* back from bottom right */
      plotpoint(image,(image->width-1),(image->height-i-1),(255-i)); /* up from bottom right */

      plotpoint(image,i,(image->height-1),(255-i)); /* across from bottom left */
      plotpoint(image,0,(image->height-i-1),(255-i)); /* up from bottom left */
    }
      ...
      for (i=0;i<image->height;i++)
        {
          for (j=0;j < image->width; j++)
            {
              image->image[i * image->width + j] = 255 - image->image[i * image->width + j];
            }
        }
    }
  ...
  if (outfile == NULL)
...

	{
		...
		pcx = EncodePCX (image->image, image->width, image->height, image->width, palette, false, &pcx_len);
