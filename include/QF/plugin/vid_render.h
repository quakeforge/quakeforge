...
typedef struct vid_draw_s {
	void (*CharBuffer) (int x, int y, struct draw_charbuffer_s *buffer);
	void (*SetScale) (int scale);
	void (*Character) (int x, int y, unsigned ch);
	void (*String) (int x, int y, const char *str);
	void (*nString) (int x, int y, const char *str, int count);
	void (*AltString) (int x, int y, const char *str);
	void (*ConsoleBackground) (int lines, byte alpha);
	void (*Crosshair) (void);
	void (*CrosshairAt) (int ch, int x, int y);
	void (*TileClear) (int x, int y, int w, int h);
	void (*Fill) (int x, int y, int w, int h, int c);
	void (*Line) (int x0, int y0, int x1, int y1, int c);
	void (*TextBox) (int x, int y, int width, int lines, byte alpha);
	void (*FadeScreen) (void);
	void (*BlendScreen) (quat_t color);
	qpic_t *(*CachePic) (const char *path, bool alpha);
	void (*UncachePic) (const char *path);
	qpic_t *(*MakePic) (int width, int height, const byte *data);
	void (*DestroyPic) (qpic_t *pic);
	qpic_t *(*PicFromWad) (const char *name);
	void (*Pic) (int x, int y, qpic_t *pic);
	void (*FitPic) (int x, int y, int width, int height, qpic_t *pic);
	void (*Picf) (float x, float y, qpic_t *pic);
	void (*SubPic) (int x, int y, qpic_t *pic, int srcx, int srcy, int width, int height);
	int (*AddFont) (struct font_s *font);
	void (*Glyph) (int x, int y, int fontid, int glyphid, int c);
