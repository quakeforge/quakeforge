...
static qfv_light_render_t
make_render  (const light_control_t *cont, bool no_style, uint32_t style)
{
						...
						exprctx_t *ectx)
{
	...
	if (ndlight) {
		...
		for (int i = 0; i < ndlight; i++) {
			...
			render[i] = make_render (r, style_disable, 0);
		}
		...
		*lframe = (lightingframe_t) {
		...
		VkDescriptorBufferInfo bufferInfo[] = {
			...
			{	.buffer = lframe->shadowmat_id_buffer,
				.offset = 0, .range = VK_WHOLE_SIZE, },
			Vulkan_Matrix_BufferInfo (ctx, i),
		...
		VkWriteDescriptorSet bufferWrite[] = {
			...
			{	.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
				...
				.descriptorCount = 1,
				.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
				.pBufferInfo = &bufferInfo[8], },
		};
		dfunc->vkUpdateDescriptorSets (device->dev, countof (bufferWrite),
									   bufferWrite, 0, 0);
...
upload_light_data (lightingctx_t *lctx, vulkan_ctx_t *ctx)
{
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		auto        r = &lctx->light_control.a[id];
		render[i] = make_render (r, style_enable, get_lightstyle (ent));
	}
	for (size_t i = 0; i < lctx->frames.size; i++) {
		auto lframe = &lctx->frames.a[i];
