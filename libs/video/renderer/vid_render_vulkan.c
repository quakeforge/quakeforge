...
{
	Vulkan_Gizmo_AddBrush (orig, bounds, num_nodes, nodes, color, vulkan_ctx);
}
...
#define IMUI_context imui_ctx
	UI_ExtendPanel (UI_PanelId (UI_FindWindow ("Renderer##menu"))) {
...
static uint32_t
vulkan_find_resource (const char *name)
{
	return QFV_TexFindTexture (vulkan_ctx, name);
}
...
static vid_model_funcs_t model_funcs = {
	...
	.find_resource = vulkan_find_resource,
...
vid_render_funcs_t vulkan_vid_render_funcs = {
	...
	.draw = {
		.CharBuffer        = vulkan_Draw_CharBuffer,
		.SetScale          = vulkan_Draw_SetScale,
		.Character         = vulkan_Draw_Character,
		.String            = vulkan_Draw_String,
		.nString           = vulkan_Draw_nString,
		.AltString         = vulkan_Draw_AltString,
		.ConsoleBackground = vulkan_Draw_ConsoleBackground,
		.Crosshair         = vulkan_Draw_Crosshair,
		.CrosshairAt       = vulkan_Draw_CrosshairAt,
		.TileClear         = vulkan_Draw_TileClear,
		.Fill              = vulkan_Draw_Fill,
		.Line              = vulkan_Draw_Line,
		.TextBox           = vulkan_Draw_TextBox,
		.FadeScreen        = vulkan_Draw_FadeScreen,
		.BlendScreen       = vulkan_Draw_BlendScreen,
		.CachePic          = vulkan_Draw_CachePic,
		.UncachePic        = vulkan_Draw_UncachePic,
		.MakePic           = vulkan_Draw_MakePic,
		.DestroyPic        = vulkan_Draw_DestroyPic,
		.PicFromWad        = vulkan_Draw_PicFromWad,
		.Pic               = vulkan_Draw_Pic,
		.FitPic            = vulkan_Draw_FitPic,
		.Picf              = vulkan_Draw_Picf,
		.SubPic            = vulkan_Draw_SubPic,
		.AddFont           = vulkan_Draw_AddFont,
		.Glyph             = vulkan_Draw_Glyph,
