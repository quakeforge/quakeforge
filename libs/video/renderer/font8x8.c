...
byte font8x8_data[] = {
	...
	0x00, 0x78, 0xCC, 0xF8, 0xCC, 0xCC, 0xF8, 0xC0,
...
#define FONT_HCHARS 16
#define FONT_VCHARS 16
#define FONT_BYTES ((FONT_VCHARS)*(FONT_HCHARS)*(FONT_HEIGHT)*(FONT_WIDTH))

qpic_t *
Draw_Font8x8Pic (void)
{
	qpic_t     *pic;
	byte       *data;
	byte       *out;
	...
	// assumes the quake palette where 254 is white, and 255 is transparent
	data = font8x8_data;
	for (j = 0; j < FONT_VCHARS; j++) {
		for (i = 0; i < FONT_HCHARS; i++) {
			for (y = 0; y < FONT_HEIGHT; y++) {
				byte        scanline = *data++;
				ind = (j * FONT_HEIGHT + y) * FONT_WIDTH * FONT_HCHARS;
				ind += i * FONT_WIDTH;
				out = pic->data + ind;
				for (x = 0; x < FONT_WIDTH; x++) {
					*out++ = 0xff - ((scanline & 0x80) >> 7);
					scanline <<= 1;
				}
			}
		}
