...
bi (IMUI_GetIO)
{
	...
	auto res = (imui_resources_t *) _res;
	auto bi_ctx = get_imui_ctx (P_INT (pr, 0));
	R_PACKED (pr, imui_io_t) = IMUI_GetIO (bi_ctx->imui_ctx);
}
...
bi (IMUI_GetKeyString)
{
	qfZoneScoped (true);
	auto res = (imui_resources_t *) _res;
	auto bi_ctx = get_imui_ctx (P_INT (pr, 0));
	if (!bi_ctx->msgbuf) {
		bi_ctx->msgbuf = MsgBuf_New (pr);
	}
	auto keystring = IMUI_GetKeyString (bi_ctx->imui_ctx);
	auto sizebuf = MsgBuf_GetSizebuf (pr, bi_ctx->msgbuf);
	sizebuf->data = (byte *) keystring->str;
	sizebuf->maxsize = keystring->truesize;
	sizebuf->cursize = keystring->size;
	R_INT (pr) = bi_ctx->msgbuf;
}
...
bi (IMUI_CheckButtonState)
{
	qfZoneScoped (true);
	auto res = (imui_resources_t *) _res;
	auto bi_ctx = get_imui_ctx (P_INT (pr, 0));
	R_INT (pr) = IMUI_CheckButtonState (bi_ctx->imui_ctx);
}

bi (IMUI_UpdateHotActive)
{
	qfZoneScoped (true);
	auto res = (imui_resources_t *) _res;
	auto bi_ctx = get_imui_ctx (P_INT (pr, 0));
	R_INT (pr) = IMUI_UpdateHotActive (bi_ctx->imui_ctx);
}
...
bi (IMUI_SetViewGravity)
{
	qfZoneScoped (true);
	auto res = (imui_resources_t *) _res;
	auto bi_ctx = get_imui_ctx (P_INT (pr, 0));
	grav_t gravity = P_INT (pr, 1);
	IMUI_SetViewGravity (bi_ctx->imui_ctx, gravity);
}
...
static builtin_t builtins[] = {
	...
	bi(IMUI_GetIO,              1, p(int)),
	bi(IMUI_GetKey,             2, p(int), p(ptr)),
	bi(IMUI_GetKeyString,       1, p(ptr)),
	...
	bi(IMUI_SetViewGravity,     2, p(int), p(int)),
