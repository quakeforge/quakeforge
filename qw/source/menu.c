...
enum { m_none, m_main, m_singleplayer, m_load, m_save, m_multiplayer, m_setup,
		...
		m_modemconfig, m_lanconfig, m_gameoptions, m_search, m_sedit } m_state;
	...
	switch (key) {
		case K_ESCAPE:
			...
			if (cls.demonum != -1 && !cls.demoplayback
				&& cls.state == ca_disconnected) CL_NextDemo ();
			break;
...
// search for known bindings
	for (i = 0; i < NUMCOMMANDS; i++) {
		...
		if (keys[0] == -1) {
			M_Print (140, y, "?" "?" "?");	// avoid any possibility of
											// trigraphs
		} else {
...
M_MultiPlayer_Draw (void)
{
	...
	if (!pingupdate && !statusupdate)
	{
		...
		if (cp->pongback)
			M_Print (STAT_X + 58, STAT_Y + 24, va("%i", (int)(cp->pongback * 1000)));
		else
